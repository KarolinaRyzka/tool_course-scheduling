...
def _computeInstructionalTime(row: Series):
    ...
    :rtype: int
    """
    startTime: datetime = pandas.to_datetime(row["CLASS START TIME"])
    endTime: datetime = pandas.to_datetime(row["CLASS END TIME"])
    ...
    time: int = endMinutes - startMinutes

    meetingPattern: str = row.get("TRAD MEETING PATTERN", "")
    daysCount: int = sum(1 for char in meetingPattern if char in "MTWRFSX")

    instructionalTime = (time * daysCount) / 50

    return instructionalTime
